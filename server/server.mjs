...
const run = (cmd, args, job) => new Promise((resolve) => {
  const p = spawn(cmd, args, { stdio: ['ignore', 'pipe', 'pipe'] });
  let out = '', err = '';
  p.stdout.on('data', d => { out += d; parseProgress(String(d), job); });
  p.stderr.on('data', d => { err += d; });
  p.on('close', code => resolve({ code, out, err }));
});

function parseProgress(s, job) {
  const m = s.match(/\[download\]\s+([\d.]+)%/g);
  if (m && job) job.progress = parseFloat(m[m.length - 1].match(/([\d.]+)%/)[1]);
}
...
async function fetchSource(job, url, id, kind) {
  ...
  if (cached) { job.step = 'using cached download'; return cached; }
  const prefix = (kind === 'audio' ? 'a_' : 'v_') + id;
  for (const args of ladder(kind)) {
    job.step = `downloading (${args.slice(0, 2).join(' ')}${args.includes('--cookies-from-browser') ? ' +cookies' : ''})`;
    job.progress = 0;
    const r = await run('yt-dlp', ['--no-playlist', '--newline', ...args, '-o', join(CACHE, prefix + '.%(ext)s'), url], job);
    const got = findCached(id, kind);
    if (r.code === 0 && got) return got;
...
async function runJob(job, { url, kind, start, end }) {
  try {
    ...
    const [id, title] = info.out.trim().split('\n');
    ...
    job.step = clip ? 'trimming' : 'encoding';
    ...
    const r = await run('ffmpeg', ['-y', '-v', 'error', ...seek, '-i', src, ...enc, out]);
    if (r.code !== 0) throw new Error('ffmpeg failed: ' + r.err.slice(-400));
    job.file = out; job.status = 'done'; job.step = 'done';
...
http.createServer((req, res) => {
  ...
  if (req.method === 'POST' && u.pathname === '/jobs') {
    ...
    return req.on('end', () => {
      ...
      const id = randomUUID();
      const job = { id, status: 'running', step: 'queued', progress: 0, log: [], file: null, error: null };
      jobs.set(id, job);
      runJob(job, b);
      json(res, 202, { id });
    });
